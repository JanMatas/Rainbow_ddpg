...
class MicoEnv(BulletRobotEnv):
    ...
    def __init__(
        ...
        neverDone=False,
        rotation="normal",
        unreliableGrasp=False,
        binGripper=False,
        continousGripper=False,
        binaryGripThreshold = 0.2,
        slow=False,
        reach=False,
    ):
        ...
        self.low_dim_space = spaces.Box(-np.inf, np.inf, shape=(22,), dtype="float32")
        ...
        self.goalShape = self.p.createCollisionShape(self.p.GEOM_SPHERE, radius=0.03)
    ...
    def state_vector(self):
        ...
        dt = self.dt
        if self.has_object:
            obj_pos = np.array(self.p.getBasePositionAndOrientation(self.body)[0])
        ...
        # [0,3] [3:6], [6:8], [8:11], [11:14], [14:17], [17:18], [18,21], [21:22]
    ...
    def _get_obs(self):
        ...
        elif self.observation_type == "her_pixels":
            return {
                "observation": pixels,
        ...
        elif self.observation_type == "her_low_dim":
            return {
                "observation": her_low_dim,
                "achieved_goal": achieved_goal.copy(),
                "desired_goal": self.goal.copy(),
            }
        elif self.observation_type == "her_composed":
            obs = {"low_dim": low_dim, "pixels": pixels}
            return {
                "observation": obs,
                "achieved_goal": achieved_goal.copy(),
                "desired_goal": self.goal.copy(),
    ...
    def _render_callback(self):
        pass
        # colShape = self.p.createCollisionShape(self.p.GEOM_SPHERE, radius=0.03)
        # self.visualizeTarget = self.p.createMultiBody(baseMass=0.0,
        #         baseCollisionShapeIndex = colShape,
        #         basePosition = self.goal,
        #         useMaximalCoordinates=0
        # )
        # self.p.changeVisualShape(self.visualizeTarget,-1, rgbaColor=[0,0,1,1])

    def _render_cleanup(self):
        pass
        # self.p.removeBody(self.visualizeTarget)
    ...
    def _reset_sim(self, state=None, stateFile=None):
        ...
        self.goalShape = self.p.createCollisionShape(self.p.GEOM_SPHERE, radius=radius)
        self.visualizeTarget = self.p.createMultiBody(
            0.0, self.goalShape, basePosition=self.goal, useMaximalCoordinates=1
        )
        color = [0.2, 0.2, 0.8, 1]
        if self.randomizeObjects:
            color = np.random.uniform([0.1, 0.1, 0.7, 1], [0.3, 0.3, 1, 1], (4,))
        ...
        if self.has_object:
            ...
            if self.randomizeObjects:
                obj_color = np.random.uniform(
                    [0.7, 0.1, 0.1, 1], [1, 0.3, 0.3, 1], (4,)
                )
    ...
    def _sample_goal(self):
        if self.fixed_goal:
            if self.target_in_the_air:
                ...
                return np.concatenate([np.random.normal([-0.3, 0.2], 0.05), [0.03]])
    ...
    def draw_goal(self):
        self.p.resetBasePositionAndOrientation(
            self.visualizeTarget, self.goal, [0, 0, 0, 1]
        )

    def _is_success_her(self, achieved_goal, desired_goal):
        d = goal_distance(achieved_goal, desired_goal)
    ...
    def initArm(self):
        ...
        if self.randomizeArm:
            spawnPos = np.clip(
                [0, 0, 0.03], [0, 0, 0.07], np.random.normal([0, 0, 0.05], 0.02)
            )
        ...
        self.p.changeVisualShape(mount, -1, rgbaColor=np.concatenate([link_color, [1]]))
        colShape = self.p.createCollisionShape(
    ...
    def initEnv(self):
        ...
        # self.planeId = self.p.loadURDF("plane.urdf",[1,0,0], self.p.getQuaternionFromEuler([0,0,2]))
        # self.wallId = self.p.loadURDF("plane.urdf",[2,0,0], self.p.getQuaternionFromEuler([0,math.pi/2,0]))
        ...
        if self.randomizeTextures:
            ...
            if self.normalTextures:
                ...
                wood_color = np.random.uniform([100, 100, 100], [130, 255, 130])
                wall_color = np.random.uniform([100, 100, 100], [130, 255, 130])
            tex1 = self.p.loadTexture(
                noise.createAndSave(
                    home + "/tmp/jm6214/wall-{}.png".format(self.envId),
                    "cloud",
                    wall_color,
                )
            )
            tex2 = self.p.loadTexture(
                noise.createAndSave(
                    home + "/tmp/jm6214/table-{}.png".format(self.envId),
                    "cloud",
                    wood_color,
                )
            )
    ...
    def get_aux(self):
        ...
        return np.concatenate(
            [self.arm.getJointPoses(), np.array(grip_state[0]), self.arm.goalPosition]
        )
        # return np.concatenate([self.arm.getJointPoses(), np.zeros((3,)), np.zeros((3,))])
