...
env_args = {
    "pusher_fix": {"env_id": "MicoEnv-pusher_fix-{}-{}-{}-{}-v1", 'entry_point':'micoenv.mico_robot_env:MicoEnv'},

}

env_kwargs = {

    "pusher_fix": {"doneAfter":300,'target_in_the_air': False, "has_object": True},
        ...
        for randomness in map(lambda a: "acton"+"".join(a), powerset(["x"])):
            for useVelos in [False]:
                for env_type in ["pusher_fix"]:
                        ...
                        env_id = env_args[env_type]["env_id"].format(obs_type, reward_type, velos_id, randomness)
...
register(
    ...
    kwargs={'target_in_the_air': False, "has_object": True, "fixed_goal": True, "use_gui":True, "randomizeArm":True, "randomizeCamera":True, "randomizeTextures":True},
)
