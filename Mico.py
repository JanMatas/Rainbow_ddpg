...
class Mico:
    def __init__(self,
        ...
        # If true, the arm will compute IK to goal position
        self.ikEnabled = not velocityControl
    ...
    def computeIKInformation(self, rotation):
        jointInformation = list(
            map(lambda i: self.p.getJointInfo(self.armId, i),
                range(self.numJoints)))
        self.IKInfo = {}
        assert all(
            [len(jointInformation[i]) == 17 for i in range(self.numJoints)])
        self.IKInfo["lowerLimits"] = list(
            map(lambda i: jointInformation[i][8], range(8)))
        self.IKInfo["upperLimits"] = list(
            map(lambda i: jointInformation[i][9], range(8)))

        self.IKInfo["jointRanges"] = list(
            map(lambda i: jointInformation[i][9] - jointInformation[i][8],
                range(8)))
        ...
        if rotation == "normal":
            ...
            self.IKInfo["restPoses"] = list(
                map(math.radians, [
                    ...
                    346.65538012555663, -309.2623679328356, 336.1405174953582,
                    0.0
                ]))
    ...
    def applyAction(self, action):
        ...
        shouldPenalize = np.any(
            self.goalPosition + action[0:3] > self.reach_high) or np.any(
    ...
    def applyVelocities(self, velocities):
        ...
        for i in range(8):
            if jointPoses[i] > self.IKInfo["upperLimits"][i] and velocities[i] > 0 or jointPoses[i] < \
                ...
                velocities[i] = 0
        for i in range(7, 10):
            if jointPoses[i] < 0 and velocities[i] < 0 or jointPoses[
                    i] > 1.4 and velocities[i] > 0:
                velocities[i] = 0
        for i in range(10):
    ...
    def ikStep(self):
        ...
        self.goalReached = all(gripperReached) and all(
            map(
                lambda i: abs(self.p.getJointState(self.armId, i)[
                              0] - jointPoses[i]) < self.goalEpsilon, range(6)
            )
        )
        ...
        if not self.graspableObject is None:
            ...
            if all([
                    pos > 0.7 for pos in self.gripperPositions
                            ...
                            ]) and leftPoints and rightPoints:
    ...
    def initializeGripper(self):
        ...
        fakeGripperObject = self.p.createCollisionShape(
        ...
        for i in range(2):  # Left and Right
            ...
            for j in range(-1, 2):  # Multiple objects for stability
                newBody = self.p.createMultiBody(
                    baseMass=0.0001,
    ...
    def initiaizeDebugSliders(self):
        ...
        self.debug_gripper = self.p.addUserDebugParameter(
            "gripper", 0, 1.45, 0)
        self.debug_camera_on = self.p.addUserDebugParameter("camera", 0, 1, 0)
        self.debug_manual_control = self.p.addUserDebugParameter(
            "manual_control", 0.0, 1.0, 0)

    def isGrasping(self):
        return len(self.gripperConstraints) > 0

    def checkDebugInputs(self):
        self.manual_control = self.p.readUserDebugParameter(
            self.debug_manual_control) > 0.5
        if self.manual_control:
            ...
            self.goalGripper = self.p.readUserDebugParameter(
                self.debug_gripper)
